...
import type { AvatarStackTokens, AvatarStackProps, AvatarStackAppearance } from './AvatarStack.types';
...
export const useAvatarStackSlotProps = (props: AvatarStackProps, tokens: AvatarStackTokens, theme: Theme) => {
...
/**
 * Helper function to set color tokens on AvatarStack. If no color tokens are passed in, then the appearance prop will be used to provide colors
...
export const colorsFromAppearance = (
  ...
  theme: Theme,
): Pick<AvatarStackTokens, 'contentColor' | 'lineColor'> => {
  switch (appearance) {
    case 'default':
      return {
 ...
 * AvatarStack is vertical and has content.
 */
